...
def ida_star(root, cost_matrix):
    threshold = min_out_heuristic(root.path[-1], cost_matrix, root.visited)
    expanded_nodes = 0
    generated_nodes = 0
    while True:
        distance, state, exp, gen = search(root, 0, threshold, cost_matrix, 0, 0)
        expanded_nodes += exp
        generated_nodes += gen
        if distance == 'FOUND':
...
def search(node, g, threshold, cost_matrix, expanded_nodes, generated_nodes):
    f = g + node.current_cost + min_out_heuristic(node.path[-1], cost_matrix, node.visited)
    if f > threshold:
    ...
    for succ in node.successors(cost_matrix):
        ...
        if temp == 'FOUND':
            return 'FOUND', state, exp, gen
        if temp < min_threshold:
            min_threshold = temp
    return min_threshold, None, exp, gen
